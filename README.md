...
```csharp
public void Inplace(matrix A11Inv, matrix A22, vector Xold, double mvar, matrix B, vector Sources, vector X)
{
    // X = A11Inv * (0.5 * mvar * B * Sources + A22 * Xold);
    X.CopyMul(0.5 * mvar, B, Sources).AddMul(A22, Xold).PreMul(A11Inv);
    // Xold = X;
    Xold.Copy(X);
}
```
